...
var getPlayerStats = function (config) {
    return function (options) { return __awaiter(void 0, void 0, void 0, function () {
        var query, _a, $, i$, m$, nameText, name, ign, imageUrl, image, age, country, team, getOverviewStats, overviewStatistics, getAdditionalInfoHeader, getAdditionalInfoSubRatings, additionalInfo, getIndivialStats, individualStatistics, matches;
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 1:
                    ...
                    getAdditionalInfoHeader = function (label) {
                        var lbl = label.toLowerCase();
                        var row = $('.summaryStatBreakdown .aboveAverage').filter(function (_, x) {
                            return x.text().toLowerCase().includes(lbl);
                        });
                        if (row.exists()) {
                    ...
                    getAdditionalInfoSubRatings = function (label) {
                        var lbl = label.toLowerCase();
                        var row = $('.g-grid .col-custom').filter(function (_, x) {
                            return x.text().toLowerCase().includes(lbl);
                        });
                        if (row.exists()) {
